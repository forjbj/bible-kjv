...
export class HistoryService {
  ...
  storeBookmark(){
    this.storedBookmarks = JSON.parse(localStorage.getItem('bookmarks')!);
    // let verseToBookmark = (document.activeElement!.id);
    // console.log(this.verseClicked)
    // let selectedVerse = [this.bibleService.testament, this.bibleService.bookSelected, this.bibleService.chapterNumber, this.bibleService.verseNumber];
    // let selectedVerse = [this.bibleService.bible[verseToBookmark[0]].books[verseToBookmark[1]].bookName this.bibleService.chapterNumber, this.bibleService.verseNumber];
    // let x = JSON.stringify([selectedVerse]);
    if (this.storedBookmarks) {
      // if (this.bibleService.menuHistoryBook == false && (this.bibleService.bookSelected != recentStore[0][1])
      //   || (this.bibleService.testament != recentStore[0][0])) {
        this.storedBookmarks.unshift(JSON.parse(this.verseClicked));
      // console.log(this.verseClicked);
      // console.log(this.storedBookmarks)
