...
export class ChapterNumbersComponent implements AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (
      localStorage.getItem("leftHanded") == "no" ||
      localStorage.getItem("leftHanded") == null
    ) {
  ...
  ngAfterViewInit() {
    ...
    this.observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.target.id) {
          //necessary as sections used everywhere in semantic html
          let chapter = entry.target!.id!;
          let splits = chapter.split("-");
          let targetChapter = splits[2];
          if (entry.isIntersecting) {
            chaptersGrid[Number(targetChapter) - 1].classList.add(
              "chapterScroll",
            );
          } else {
            chaptersGrid[Number(targetChapter) - 1].classList.remove(
              "chapterScroll",
            );
          }
