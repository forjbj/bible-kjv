...
pub fn search(searches: usize, inp: String, acc: usize) -> String {
    ...
    match search_num {
        0 => {
          results_fin = format!("<header>There are no results for \"{}\".<br><br>Please check the spelling, or try part of a word and set the last search option to 'Word Contains Characters'.</header>", inp_str);
        }
        1 => {
            results_fin = format!(
